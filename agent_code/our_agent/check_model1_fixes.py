...
from .rewards import (
    detect_custom_events, BOMB_NEXT_TO_CRATE, USELESS_BOMB, BOMB_WITH_NO_ESCAPE,
    SAFE_BOMB_THREATENS_OPPONENT, SAFE_BOMB_TRAPS_OPPONENT,
)

BOMB_EVENTS = {
    BOMB_NEXT_TO_CRATE, USELESS_BOMB, BOMB_WITH_NO_ESCAPE,
    SAFE_BOMB_THREATENS_OPPONENT, SAFE_BOMB_TRAPS_OPPONENT,
}
...
def check_invalid_bomb_gets_no_bomb_shaping():
    ...
    ok = not leaked
    print('OK: invalid BOMB gets no bomb-quality reward.' if ok
...
def check_successful_bomb_gets_safe_threat_reward():
    ...
    print('OK: successful escapable threat is rewarded.' if ok
