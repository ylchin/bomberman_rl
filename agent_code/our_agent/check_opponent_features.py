'''
Verifies OPP_TRAPPED / OPP_NEAR_DEADEND / is_dead_end / opponent_trapped against hand-built boards 
with a known right answer, so we're not just trusting the code compiles.
Run from the repo root: python -m agent_code.our_agent.check_opponent_features
'''

import numpy as np
from .features import (is_dead_end, opponent_trapped, state_to_features, FEATURE_NAMES,)
...
def check_state_to_features_wires_it_through():
    #confirms the feature vector's indices actually reflect the board, not just that the standalone functions work in isolation
    ...
    if ok:
        print("OK: state_to_features correctly wires OPP_TRAPPED/OPP_NEAR_DEADEND through to indices 32/33.")
    return ok
