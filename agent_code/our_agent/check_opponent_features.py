...
def check_actual_bomb_geometry():
    ...
    if ok:
        print(
            "OK: opponent_trapped uses our actual proposed bomb and time-aware escape."
        )
    return ok
...
def check_feature_wiring():
    ...
    if ok:
        print(
            "OK: corrected opponent + explicit safe-bomb interaction features are wired through."
        )
    return ok
