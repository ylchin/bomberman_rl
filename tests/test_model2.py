...
from agent_code.our_agent.model_net import NetQ, INPUT_DIM, encode_state, transform_input
...
def board():
    ...
    field[:, [0, -1]] = -1
    field[5, 6] = 1
    return dict(field=field, coins=[(4, 3)], bombs=[((7, 8), 3)],
                explosion_map=np.zeros_like(field), others=[("opponent", 0, True, (9, 2))],
                self=("agent", 0, True, (2, 3)), step=1, round=1)
...
class Model2Tests(unittest.TestCase):
        ...
        with torch.no_grad():
            for network in (model.online, model.target):
                for parameter in network.parameters():
                    parameter.zero_()
            model.online.layers[-1].bias.copy_(torch.tensor([0., 3., 0., 0., 0., 0.]))
            model.target.layers[-1].bias.copy_(torch.tensor([10., 2., 0., 0., 0., 0.]))
    ...
    def test_learning_target_sync_and_checkpoint(self):
        ...
        frozen = {key: value.clone() for key, value in model.target.state_dict().items()}
    ...
    def test_n_step_returns_and_terminal_tail(self):
        agent = SimpleNamespace(feature_dim=INPUT_DIM, episode=0,
                                t=dict(gamma=0.5, n_step=2, use_symmetry=False),
                                buffer=Mock(), _sym=None)
    ...
    def test_survivor_final_step_not_duplicated(self):
        phi = np.zeros(INPUT_DIM, dtype=np.float32)
        agent = SimpleNamespace(traj=[(phi, 0, 1.0)], ep_reward=1.0,
                                ep_counts=train.Counter(), episode=0,
                                t=dict(learn_iters_end=0), model=Mock(),
                                _weights_path="unused")
        with patch.object(train, "_flush_episode_to_buffer") as flush, \
