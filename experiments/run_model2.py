...
def main():
    ...
    files = list(source_root.glob("*.py")) + list(
        (source_root / "agent_code").rglob("*.py")
    )
    ...
    def execute(command, label, process_env):
        ...
        with (
            (output / f"{label}.stdout.log").open("w") as stdout,
            (output / f"{label}.stderr.log").open("w") as stderr,
        ):
            process = subprocess.Popen(
                command, cwd=source, env=process_env, stdout=stdout, stderr=stderr
            )
            save_status(child_pid=process.pid)
            code = process.wait()
        if code:
            raise RuntimeError(
                f"{label} failed with exit code {code}; inspect its logs"
            )
    ...
    try:
        execute(
            [
                ...
                str(args.seed),
                "--save-stats",
                str(output / "training_stats.json"),
            ],
            "training",
            env,
        )
        ...
        for candidate, suffix in (("best", "_best"), ("latest", "")):
            ...
            execute(
                [
                    ...
                    "--out",
                    str(output / f"eval_{candidate}.csv"),
                    "--keep-stats-files",
                ],
                f"evaluate_{candidate}",
                evaluation_env,
            )
        ...
        shutil.copy2(
            source / "agent_code" / "our_agent" / f"training_net_{run_name}.csv",
            output / "training.csv",
        )
        save_status(
            phase="complete", finished_utc=datetime.now(timezone.utc).isoformat()
        )
